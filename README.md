...
**Viewer login history**: every successful Viewer login (the display name they typed + timestamp) is written to the `viewer_login_logs` table and exposed at `GET /api/auth/viewer-logs` - Master-only (`403` for Viewers). The frontend surfaces this as the "Logs" button in the top bar (visible to Master only).
...
**Why Tiptap**: the editor (bold/italic/underline/strike/highlight, text alignment, headings 1-3, bullet/numbered lists) is a React component (`frontend/components/NoteEditorModal.jsx`) built on [Tiptap](https://tiptap.dev)/ProseMirror. All rendering, formatting state, and editing logic run in the browser - the backend's job is limited to validating and storing/retrieving a JSON string in SQLite and regenerating the plaintext extract, the same negligible-CPU operations as any other file metadata write. There is no server-side HTML rendering or text processing of any kind, keeping notes as lightweight on the Pi as the existing file endpoints.
...
| `GET` | `/api/notes/{id}/export` | Master or Viewer | Streams `content_plaintext` as a `.txt` file, same download pattern as files. Subject to the same visibility check. |
...
- **Every** create/update walks the submitted ProseMirror document (`app/notes_validation.py::validate_note_content`) against an explicit allow-list of node types (`doc`, `paragraph`, `text`, `heading`, `bulletList`, `orderedList`, `listItem`, `hardBreak`) and mark types (`bold`, `italic`, `underline`, `strike`, `highlight`). Anything else - including any raw-HTML-shaped node - is rejected with `422` and nothing is saved.
- Heading levels are restricted to 1-3, `textAlign` to `left`/`center`/`right`/`justify`, and a highlight's `color` attribute must be a hex color, matching exactly what the toolbar can produce.
...
`tests/test_notes.py` covers all of this: RBAC (`403` for a Viewer attempting create/update/delete), the private-folder visibility rule (hidden from listings, `404` on direct `GET`), a disallowed node type and a disallowed mark type each rejected with `422` and unsaved, an oversized payload rejected with `422`, and `content_plaintext` regeneration after an update. It runs alongside the RBAC suite below.
